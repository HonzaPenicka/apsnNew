...
export default function Navbar() {
  ...
  return (
    <nav className="h-24 mt-4 p-4 flex justify-between items-center bg-white/20 font-medium text-2xl my-4">
      ...
      <div className="space-x-4 md:hidden justify-center items-center flex">
        <button onClick={toggleMenuServices}>
          <BurgerMenu />
          {openMenuServices && (
                <div className="flex flex-col z-10 space-y-4 absolute top-28 right-4 bg-white/20 shadow-lg py-8 w-2/3 container mx-auto text-right px-4 overflow-y-auto max-h-[calc(100vh-5rem)]">                  <Link href="/" className="hover:text-yellow-600">Domů</Link>
                  <Link href="/about" className="hover:text-yellow-600">O nás</Link>
                  <Link href="/about" className="hover:text-yellow-600">Služby</Link>
                  <Link href="/contact" className="hover:text-yellow-600">Kontakt</Link>
                </div>
