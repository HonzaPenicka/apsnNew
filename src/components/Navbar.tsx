...
export default function Navbar({ isActive }: { isActive?: boolean }) {
  ...
  return (
    <nav
      ref={containerRef}
      className="h-24 mt-4 p-4 flex justify-between items-center font-medium text-2xl my-4"
    >
      <Link 
        data-animate
        href="/" 
      ...
      >
        <button onClick={toggleMenuServices}>
          {openMenuServices && (
            <div className="flex flex-col z-10 space-y-4 absolute top-24 right-0.5 shadow-lg py-4 w-2/3 container mx-auto text-right px-4 overflow-y-auto max-h-[calc(100vh-5rem)]">
